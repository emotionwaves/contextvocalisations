...
button_positions = data_map/np.max(data_map,0)*500 + 110
button_positions[:,0] = button_positions[:,0]*1.75 + 50
...
for label_ix, label in enumerate(labels):
    ...
    label_positions.append(np.mean(button_pos_ix,0) + (np.mean(button_pos_ix,0) - button_pos_mean)*.5)

all_label_html_tag = []
for label_ix, label in enumerate(labels):
    formatted_label = LABEL_TEMPLATE.format(class_color = COLOR_LIST[label_ix],
                                            #label_color = COLOR_LIST[label_ix],
